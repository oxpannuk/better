...
// === ВСЕ КОМПАНИИ (больше не привязаны к городу) ===
...
// === Офисы по компании (name убран из offices) ===
...
if ($_POST['action'] === 'delete' && isset($_POST['id'])) {
    $id = (int)$_POST['id'];
    $owner_id = $pdo->prepare("SELECT user_id FROM messages WHERE id = ?")
                    ->execute([$id]) ? $pdo->fetchColumn() : 0;
